...
@section('content')
    ...
    <div class="mb-6 flex items-center gap-2 text-sm text-slate-500">
        ...
        <span>/</span>
        <span class="text-slate-800">Edit {{ $lesson->title }}</span>
    </div>

    <div class="rounded-2xl bg-white shadow-sm border border-slate-200">
        <div class="border-b border-slate-200 px-6 py-4">
            <h2 class="text-lg font-bold text-slate-800">Edit Lesson</h2>
            <p class="text-xs text-slate-500 mt-0.5">Module: {{ $module->title }} &mdash; Course: {{ $course->title }}</p>
        </div>

        <div class="px-6 py-6">
            <form method="POST" action="{{ route('admin.courses.modules.lessons.update', [$course, $module, $lesson]) }}">
                @csrf
                @method('PUT')
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title', $lesson->title) }}" required maxlength="255"
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="slug" id="slug" value="{{ old('slug', $lesson->slug) }}" maxlength="255"
                           class="mt-1 block w-full rounded-lg border border-slate-300 px-3 py-2 text-sm shadow-sm focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500">
                ...
                <div class="mb-4">
                    ...
                    <input type="url" name="video_url" id="video_url" value="{{ old('video_url', $lesson->video_url) }}" maxlength="255"
                           class="mt-1 block w-full rounded-lg border border-slate-300 px-3 py-2 text-sm shadow-sm focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500"
                ...
                <div class="mb-4">
                    ...
                    <textarea name="content" id="content" rows="8"
                              class="mt-1 block w-full rounded-lg border border-slate-300 px-3 py-2 text-sm shadow-sm focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500"
                              placeholder="Write lesson content here...">{{ old('content', $lesson->content) }}</textarea>
                ...
                <div class="mb-6">
                    <label class="inline-flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" name="is_free_preview" value="1" @checked(old('is_free_preview', $lesson->is_free_preview))
