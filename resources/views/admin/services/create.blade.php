...
@section('content')
    <div class="rounded-2xl bg-white shadow-sm border border-slate-200">
        <div class="border-b border-slate-200 px-6 py-4">
            <h2 class="text-lg font-bold text-slate-800">Add New Service</h2>
        </div>

        <form method="POST" action="{{ route('admin.services.store') }}" class="px-6 py-6">
            @csrf
            @include('admin.services._form', ['service' => null, 'nextOrder' => $nextOrder])
