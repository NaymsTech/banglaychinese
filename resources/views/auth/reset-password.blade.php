...
<head>
    ...
    <meta name="description" content="Set a new password for your {{ $siteName }} account and get straight back to learning Chinese.">
            ...
            <div class="relative flex flex-col justify-between gap-16 px-12 py-14 xl:px-16">
                ...
                <div>
                    <h1 class="max-w-md text-4xl font-extrabold leading-tight font-display xl:text-5xl">
                        Almost there — set your new <span class="text-emerald-400">password</span>
                    </h1>
                    <p class="mt-4 max-w-md text-lg text-emerald-100/80">
                        Choose a strong password you haven't used before, then you'll be straight back into your courses.
                    </p>
                    ...
                    <ul class="mt-10 space-y-4 text-emerald-50">
                        <li class="flex items-center gap-3">
                            ...
                            <span class="text-base">At least 8 characters long</span>
                        </li>
                        <li class="flex items-center gap-3">
                            <span class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-emerald-500/20 text-emerald-300">
                                <svg class="h-4 w-4" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg>
                            </span>
                            <span class="text-base">Keep it unique to this account</span>
                        </li>
                        <li class="flex items-center gap-3">
                            <span class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-emerald-500/20 text-emerald-300">
                                <svg class="h-4 w-4" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg>
            ...
            <div class="w-full max-w-md">
                ...
                <div class="rounded-2xl bg-white p-8 shadow-xl sm:p-10">
                    ...
                    <form method="POST" action="{{ route('password.store') }}" class="mt-8 space-y-6">
                        ...
                        {{-- Password reset token + email from the reset link --}}
                        <input type="hidden" name="token" value="{{ $request->route('token') }}">
                        <input type="hidden" name="email" value="{{ old('email', $request->email) }}">

                        {{-- New password --}}
                        <div x-data="{ show: false }">
                            <label for="password" class="mb-1.5 block text-sm font-semibold text-gray-700">New Password</label>
                            <div class="relative">
                                ...
                                <input id="password" type="password" name="password"
                                       ...
                                       required autofocus autocomplete="new-password" placeholder="Minimum 8 characters"
                ...
                <p class="mt-4 flex items-center justify-center gap-1.5 text-xs text-slate-400">
                    ...
                    Your new password is encrypted before it is stored
                </p>
            </div>
        </section>
    </div>
