...
@php
    ...
    $isService = $course->type === 'service';
    $hasConsultation = !empty($course->consultation_link);

    // Dynamic type-based badge
    $badge = match ($course->type) {
        'service' => ['label' => 'Study in China', 'filter' => 'study-in-china', 'icon' => '🎓'],
        default   => match ($course->slug) {
    ...
    // Gradient — service gets a premium gold gradient, courses keep green
    ...
    // CTA link — services with consultation go to WhatsApp, otherwise course show page
    $ctaLink = $isService && $hasConsultation
        ? $course->consultation_link
        : route('courses.show', $course->slug);

    $ctaLabel = $isService ? 'ফ্রি কনসালটেশন' : 'ভর্তি হন';
    $ctaTarget = $isService && $hasConsultation ? '_blank' : '_self';
    $ctaRel = $isService && $hasConsultation ? 'noopener' : '';
@endphp
...
>
    ...
    <div class="relative h-40 {{ $headerGradient }} p-6">
        ...
        <div class="flex items-start justify-between">
            ...
            @if($course->is_featured)
                @if($isService)
                    <span class="rounded-full bg-amber-400 px-2.5 py-1 text-xs font-extrabold text-amber-950">★ PREMIUM</span>
                @else
        ...
        <div class="mt-10 flex flex-wrap items-center gap-2">
            @if(!$isService && $course->hsk_level)
                <span class="rounded-lg bg-white/20 px-2.5 py-1 text-xs font-bold text-white">HSK {{ $course->hsk_level }}</span>
            @endif
            <span class="rounded-lg {{ $isService ? 'bg-amber-500' : 'bg-accent-600' }} px-2.5 py-1 text-xs font-extrabold text-white">
                @if($course->price > 0)
            ...
            @if($isService)
                <span class="rounded-lg bg-white/20 px-2.5 py-1 text-xs font-bold text-amber-100">সার্ভিস</span>
            @endif
            @if($course->duration_weeks && !$isService)
    ...
    <div class="flex flex-1 flex-col p-6">
        ...
        {{-- Feature list — services show different benefits --}}
        @if($isService)
            <ul class="mt-4 space-y-2 text-sm text-slate-600">
                <li class="flex items-center gap-2">
                    <svg class="h-4 w-4 shrink-0 text-amber-600" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/></svg>
                    এক্সপার্ট গাইডেন্স
                </li>
                <li class="flex items-center gap-2">
                    <svg class="h-4 w-4 shrink-0 text-amber-600" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/></svg>
                    সম্পূর্ণ ডকুমেন্ট সাপোর্ট
                </li>
            </ul>
        @else
            <ul class="mt-4 space-y-2 text-sm text-slate-600">
        ...
        @if($isService)
            <div class="mt-4 flex items-center gap-2 rounded-xl bg-amber-50 px-3 py-2 text-xs font-semibold text-amber-800">
                <span class="flex h-5 w-5 items-center justify-center rounded-full bg-amber-200 text-xs">🔥</span>
                <span>সীমিত সময়ের জন্য ফ্রি কনসালটেশন</span>
            </div>
        ...
        <div class="mt-4 grid grid-cols-2 gap-3">
            @if($isService)
                {{-- Service: "Details" + "Free Consultation" (WhatsApp) --}}
                <a href="{{ route('courses.show', $course->slug) }}"
                   class="inline-flex items-center justify-center gap-1.5 rounded-full border-2 border-amber-700 px-4 py-2.5 text-sm font-bold text-amber-700 transition hover:bg-amber-50">
                    বিস্তারিত
                ...
                <a href="{{ $ctaLink }}"
                   target="{{ $ctaTarget }}" rel="{{ $ctaRel }}"
                   class="inline-flex items-center justify-center gap-1.5 rounded-full bg-amber-600 px-4 py-2.5 text-sm font-bold text-white shadow-lg shadow-amber-600/25 transition hover:bg-amber-700 animate-pulse">
                    {{ $ctaLabel }}
