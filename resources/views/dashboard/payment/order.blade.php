        ...
        <div class="w-full max-w-lg">
            <div class="rounded-2xl bg-white p-8 shadow-xl sm:p-10">
                ...
                <p class="mt-2 text-sm leading-relaxed text-slate-500">
                    অর্ডার <strong>#{{ $order->id }}</strong> — {{ $order->product?->title }}। সঠিক ট্রানজেকশন আইডি দিয়ে আবার জমা দিন।
                </p>
                ...
                <form method="POST" action="{{ route('dashboard.payments.order.resubmit', $order) }}" class="mt-8 space-y-5">
                    @csrf
                    <div>
                        <label for="trx_id" class="mb-1.5 block text-sm font-semibold text-gray-700">ট্রানজেকশন আইডি</label>
                        <input id="trx_id" type="text" name="trx_id" value="{{ old('trx_id', $reviewTrxId) }}"
